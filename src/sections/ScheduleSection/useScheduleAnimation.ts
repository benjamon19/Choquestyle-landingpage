...
export default function useScheduleAnimation() {
  ...
  const handleMobileClick = (day: string, rowIndex: number) => {
  ...
  const handleCellEnter = (day: string, rowIndex: number) => {
  ...
  const getCellClasses = (hasContent: boolean, day: string, rowIndex: number) => {
    const cellKey = `${day}-${rowIndex}`;
